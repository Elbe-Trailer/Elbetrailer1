...
import type { Accessory, AccessoryCost } from "@/types/database";
...
export default async function EditAccessoryPage({ params }: Props) {
  ...
  const [{ data: row }, { data: categories }, { data: cost }] = await Promise.all([
  ...
  return (
    <div className="space-y-8">
      ...
      <AccessoryForm
        ...
        cost={(cost as AccessoryCost | null) ?? null}
      />
      <PriceIncreasePanel
        kind="accessory"
        id={id}
        currentVkCents={accessory.price_adjustment_cents}
        currentEkNetCents={
          (cost as AccessoryCost | null)?.purchase_price_net_cents ?? null
        }
      />
