...
import { importListings, LISTING_COLUMNS } from "@/lib/admin/bulk-import";
...
export async function POST(req: Request) {
  ...
  const parsed = await parseUploadedWorkbook(buffer, LISTING_COLUMNS);
  if (!parsed.ok) {
  ...
  const report = await importListings(admin.supabase, parsed.rows);
